...
from .settings import pynag_directory, eventhandlers, \
        _UNRESOLVED_MACRO, _CUSTOM_VARIABLE_PREFIX, AttributeList
...
class ObjectRelations(object):
    ...
    def _expand_regex(dictionary, full_list):
        ...
        True
        """
        if settings.config.get_cfg_value('use_true_regexp_matching') == "1":
...
class ObjectFetcher(object):
    ...
    def reload_cache(self):
        ...
        if (settings.config is None or 
                settings.config.cfg_file != settings.cfg_file) :
            settings.config = pynag.Parsers.config_parser.Config(settings.cfg_file)
        if settings.config.needs_reparse():
        ...
        for object_type, objects in settings.config.data.items():
    ...
    def needs_reload(self):
        ...
        return settings.config.needs_reparse()
    ...
    def get_object_types(self):
        ...
        if settings.config is None or settings.config.needs_reparse():
            self.reload_cache()
        return settings.config.get_object_types()
...
class ObjectDefinition(object):
    ...
    def __init__(self, item=None, filename=None, **kwargs):
        ...
        if item is None:
            item = settings.config.get_new_item(object_type=self.object_type, 
                                                filename=filename)
            self.is_new = True
        else:
            self.is_new = False
    ...
    def save(self, filename=None):
        ...
        if self.is_new is True or self._filename_has_changed:
            ...
            settings.config.item_add(self._original_attributes, self.get_filename())

        # If we get here, we are making modifications to an object
        else:
            number_of_changes = 0
            for field_name, new_value in self._changes.items():
                save_result = settings.config.item_edit_field(
    ...
    def reload_object(self):
        ...
        old_me = settings.config.get_new_item(self.object_type, self.get_filename())
        old_me['meta']['defined_attributes'] = self._defined_attributes
        for k, v in self._defined_attributes.items():
            old_me[k] = v
        for k, v in self._changes.items():
            old_me[k] = v
        i = settings.config._apply_template(old_me)
    ...
    def rewrite(self, str_new_definition=None):
        ...
        settings.config.item_rewrite(self._original_attributes, str_new_definition)
        self['meta']['raw_definition'] = str_new_definition
        self._event(level='write', message="Object definition rewritten")

        # this piece of code makes sure that when we current object contains all current info
        new_me = settings.config.parse_string(str_new_definition)
        if new_me:
            new_me = new_me[0]
    ...
    def delete(self, recursive=False, cleanup_related_items=True):
        ...
        if recursive is True:
            ...
            pass
        result = settings.config.item_remove(self._original_attributes)
    ...
    def get_macro(self, macroname, host_name=None, contact_name=None):
        ...
        if macroname.startswith('$USER'):
            ...
            return settings.config.get_resource(macroname)
...
class Host(ObjectDefinition):
    ...
    def acknowledge(self, sticky=1, notify=1, persistent=0, author='pynag', comment='acknowledged by pynag',
        ...
        pynag.Control.Command.acknowledge_host_problem(host_name=self.host_name,
                                                       ...
                                                       comment=comment,
                                                       timestamp=timestamp,
                                                       command_file=settings.config.get_cfg_value('command_file')
                                                       )
    ...
    def get_current_status(self):
        ...
        status = pynag.Parsers.status_dat.StatusDat(cfg_file=settings.cfg_file)
