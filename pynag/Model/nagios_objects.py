...
class ObjectFetcher(object):
    ...
    def reload_cache(self):
        ...
        for object_type, objects in config.data.items():
            ...
            Class = pynag.Model.string_to_class.get(object_type)
            for i in objects:
                i = Class(item=i)
...
class ObjectDefinition(object):
    ...
    def copy(self, recursive=False, filename=None, **args):
        ...
        new_object = pynag.Model.string_to_class[self.object_type](filename=filename)
        for k, v in self._defined_attributes.items():
            new_object[k] = v
        for k, v in self._changes.items():
            new_object[k] = v
    ...
    def get_effective_parents(self, recursive=False, cache_only=False):
        ...
        if not self.use:
            return []
        results = []
        use = pynag.Utils.pynag.Utils.AttributeList(self.use)
    ...
    def attribute_appendfield(self, attribute_name, value):
        ...
        Example:
           ...
           """
        aList = pynag.Utils.AttributeList(self[attribute_name])
    ...
    def _do_relations(self):
        ...
        """
        parents = pynag.Utils.AttributeList(self.use)
...
class Service(ObjectDefinition):
    ...
    def _do_relations(self):
        ...
        # Do hostgroups
        hg = pynag.Utils.AttributeList(self.hostgroup_name)
        for i in hg.fields:
        ...
        sg = pynag.Utils.AttributeList(self.servicegroups)
        for i in sg.fields:
        ...
        hosts = pynag.Utils.AttributeList(self.host_name)
...
class Contact(ObjectDefinition):
    ...
    def _do_relations(self):
        super(self.__class__, self)._do_relations()
        groups = pynag.Utils.AttributeList(self.contactgroups)
...
class Contactgroup(ObjectDefinition):
    ...
    def _do_relations(self):
        super(self.__class__, self)._do_relations()
        members = pynag.Utils.AttributeList(self.members)
        ...
        groups = pynag.Utils.AttributeList(self.contactgroup_members)
        for i in groups.fields:
...
class Hostgroup(ObjectDefinition):
    ...
    def _do_relations(self):
        ...
        groups = pynag.Utils.AttributeList(self.hostgroup_members)
        for i in groups.fields:
...
class Servicegroup(ObjectDefinition):
    ...
    def _do_relations(self):
        ...
        members = pynag.Utils.AttributeList(self.members).fields
        while len(members) > 1:
        ...
        groups = pynag.Utils.AttributeList(self.servicegroup_members)
        for i in groups.fields:
